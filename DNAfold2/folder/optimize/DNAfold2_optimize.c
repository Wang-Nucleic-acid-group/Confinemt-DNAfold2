...
#pragma omp threadprivate(length1,length2,length3,length4,length5,Bs,ucs,uucs)
//FILE *fp4;
int main()
{
	...
	FILE *fpb,*fpb1;
 	...
 	for(i=0;i<N_thread;i++)
 	{
     	...
     	for (i=0;i<N_thread;i++) 
     	{
      	...
      	while(!feof(fpcfig))
      	{
      		if (fscanf(fpcfig, "%d %d %d %d %d %f\n", &ct, &ca, &cb, &cc, &cd, &rrc) != 6) 
		{
        ...
        for(i=0;i<N_thread;i++)
      	{
      		total[i]=cb;ccc[i]=cc;ccd[i]=cd;rc[i]=rrc;
      	}   
	(void)result;
      	fclose(fpcfig);      
}
...
void center()
{
	...
	for(i=1;i<=rp;i++)
	{
		cenx=cenx+rx0[i];ceny=ceny+ry0[i];cenz=cenz+rz0[i];
	}
	cenx=cenx/rp;ceny=ceny/rp;cenz=cenz/rp;
	float rd=0,rd0=0;
	for(i=1;i<=rp;i++)
	{
		rx0[i]=rx0[i]-cenx;
		ry0[i]=ry0[i]-ceny;
		rz0[i]=rz0[i]-cenz;
		rd0=sqrt(rx0[i]*rx0[i]+ry0[i]*ry0[i]+rz0[i]*rz0[i]);
		rd=rd0+rd;
	}
...
void MC_Each_Step()
{
 	...
 	void Metropolis(); 
   	...
   	uco=0.0;uuco=0.0;
   	utrs=0.0;uutrs=0.0;
   	Energy=0;Move=0; ucs=0.0;uucs=0.0;                     //if Energy=1, fuction ENERGY() is running 
   	ed=0.0;ucs=0.0;uucs=0.0;
       /*****************固定中心原子*********************/
   	re:;
...
void Metropolis(void)
{
   	int i;
   	float p;
   	u=ulj+uN*Nstrength+us+uc+(ub*NS1+ue*NS1+ud*NS2)*0.5963+uco+utrs+ucs;
   	uu=uulj+uuN*Nstrength+uus+uuc+(uub*NS1+uue*NS1+uud*NS2)*0.5963+uuco+uutrs+uucs;
...
float PC(int i1,float x1[1000],float y1[1000],float z1[1000])
{
     	...
     	if(c[i1]==1&&c[i1-3]==1&&a[i1]==1&&a[i1-3]==1)	{ul=196.4*(d-3.95)*(d-3.95);}
     	else						{ul=196.4*(d-3.95)*(d-3.95);}
     	return ul;
}
float CP(int i1,float x1[1000],float y1[1000],float z1[1000])
{
     	...
     	if(c[i1]==1&&c[i1+3]==1&&a[i1]==1&&a[i1+3]==1)	{ul=141*(d-3.95)*(d-3.95);}
     	else						{ul=141*(d-3.95)*(d-3.95);}
     	return ul;
}
float CN(int i1,float x1[1000],float y1[1000],float z1[1000])
{
     	...
     	if(c[i1]==1&&a[i1]==1)	{ul=91.6*(d-3.55)*(d-3.55);}
     	else			{ul=91.6*(d-3.55)*(d-3.55);}
     	return ul;
}
float PCP(int i1,float x1[1000],float y1[1000],float z1[1000])
{
      	...
      	else if (w>=1.0) 
     	{
      	...
      	if(c[i1]==1&&a[i1]==1)	{ue0=19.6*(a1-2.1)*(a1-2.1);}
      	else			{ue0=19.6*(a1-2.1)*(a1-2.1);}
      	return ue0;
}
float CPC(int i1,float x1[1000],float y1[1000],float z1[1000])
{
        ...
        if(c[i1]==1&&a[i1]==1)	{ue0=17.2*(a1-1.8)*(a1-1.8);}
        else			{ue0=17.2*(a1-1.8)*(a1-1.8);}
      	return ue0;
}
float PCN(int i1,float x1[1000],float y1[1000],float z1[1000])
{
      	...
      	if(c[i1]==1&&a[i1]==1)	{ue0=13.0*(a1-1.7)*(a1-1.7);}
      	else			{ue0=13.0*(a1-1.7)*(a1-1.7);}
      	return ue0;
}
float NCP(int i1,float x1[1000],float y1[1000],float z1[1000])
{
     	...
     	if(c[i1]==1&&a[i1]==1)	{ue0=28.6*(a1-1.7)*(a1-1.7);}
     	else			{ue0=28.6*(a1-1.7)*(a1-1.7);}
     	return ue0;
}
float PCPC(int i1,float x1[1000],float y1[1000],float z1[1000])
{
	...
	c3=((x1[i1-2]-x1[i1-1])*(y1[i1-1]-y1[i1+1])-(y1[i1-2]-y1[i1-1])*(x1[i1-1]-x1[i1+1]));
 	...
 	if(c[i1]==1&&a[i1]==1)	{ud0=2.6*((1-cos(di-2.5))+0.5*(1-cos(3.*(di-2.5))));}
...
float CPCP(int i1,float x1[1000],float y1[1000],float z1[1000])
{
   	float c1,c2,c3,p1,p2,p3,e1,f1,pp1,g1,g2,g3,gg1,hh1,di,ud0=0.0;
    	...
    	if (pp1<=-1.0) {di=-3.14;}
     	...
     	if(c[i1]==1&&a[i1]==1)	{ud0=8.0*((1-cos(di+2.9))+0.5*(1-cos(3.*(di+2.9))));}
...
float CPCN(int i1,float x1[1000],float y1[1000],float z1[1000])
{
  	float c1,c2,c3,p1,p2,p3,e1,f1,pp1,g1,g2,g3,gg1,hh1,di,ud0=0.0;
     	...
     	if(c[i1]==1&&a[i1]==1)	{ud0=6.4*((1-cos(di+1.3))+0.5*(1-cos(3.*(di+1.3))));}
     	else			{ud0=6.4*((1-cos(di+1.3))+0.5*(1-cos(3.*(di+1.3))));}
   	return ud0;
}
float NCPC(int i1,float x1[1000],float y1[1000],float z1[1000])
{
 	float c1,c2,c3,p1,p2,p3,e1,f1,pp1,g1,g2,g3,gg1,hh1,di,ud0=0.0;
   	...
   	if (pp1<=-1.0) 
  	{
   	...
   	if(c[i1]==1&&a[i1]==1)		{ud0=3.2*((1-cos(di+1.6))+0.5*(1-cos(3.*(di+1.6))));}
   	else				{ud0=3.2*((1-cos(di+1.6))+0.5*(1-cos(3.*(di+1.6))));}
   	return ud0;
}
/*  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  */
...
float HB(int i1,int j1,float x1[10000],float y1[10000],float z1[10000])
{
   	...
   	if (d>=8.55&&d<=9.39)   //The pairing formation condition
   	{
        	...
        	if ((type[i1]=='A'&&type[j1]=='T')||(type[i1]=='T'&&type[j1]=='A'))  {hb=-2.10;}
    	...
    	for (i=1;i<=N0;i++)  
    	{
     	...
     	{   	    
      		for(j=i+12;j<=N0;j=j+3)
             	{          		     
                    	if ((type[i]=='G'&&type[j]=='C')||(type[i]=='C'&&type[j]=='G')||(type[i]=='A'&&type[j]=='T')||(type[i]=='T'&&type[j]=='A')||(type[i]=='G'&&type[j]=='U')||(type[i]=='U'&&type[j]=='G'))
                    	{
...
void confinement()
{
	...
	for(i=1;i<=N0;i++)
	{
		Rc=sqrt(x[i]*x[i]+y[i]*y[i]+z[i]*z[i]);
		RRc=sqrt(xx[i]*xx[i]+yy[i]*yy[i]+zz[i]*zz[i]);
		if(Rc>rc[thread_num]) 
		{
			ucs=ucs+10000;
			//printf("%f\n",Rc);
		}
		if(RRc>rc[thread_num])
		{
			uucs=uucs+10000;
		}
	}
