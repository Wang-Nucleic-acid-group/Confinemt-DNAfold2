...
int main()
{
	...
	void Put_File(),Fixed_Atom(),MC_Annealing(),OutputPara(),center();
...
void center()
{
	...
	for(i=1;i<=N0;i++)
	{
		cenx=cenx+x0[i];ceny=ceny+y0[i];cenz=cenz+z0[i];
	}
...
void MC_Each_Step()
{
 	...
 	void Metropolis(); 
   	...
   	Move=0; ucs=0.0;uucs=0.0;    //if Move_3nt=1, 3nt fragment moves;
...
void Metropolis(void)
{
   	int i;
   	float p;
   	u=ulj+uN+us+uc+(ub+ue+ud)*0.5963+uco+ucs/*+uTri+umis+udang*/;
   	uu=uulj+uuN+uus+uuc+(uub+uue+uud)*0.5963+uuco+uucs/*+uuTri+uumis+uudang*/;
   	du=uu-u; 
