...
#define cut 1.60
#define nm_frag 5
FILE *fragA[20],*fragG[20],*fragC[20],*fragT[20],*CG,*out_AA,*out_rmsd;
FILE *inpdb, *outpdb;
char atom[20][1000],typen_A[20][100][4],typen_G[20][100][4],typen_C[20][100][4],typen_T[20][100][4],type[500][4],typen_AA[20][500][4];
char type_A[20][100][5],type_G[20][100][5],type_C[20][100][5],type_T[20][100][5],type_AA[20][500][5];
float x_A[20][5][100],x_G[20][5][100],x_C[20][5][100],x_T[20][5][100],x[5][500];
int id[20],idnu[20],duo1,duo2,NA[20],NG[20],NC[20],NT[20],N,N_AA[20],nucl_AA,atom_AA[20][500],nucl_AA1,duange;
...
float RMSD[20];
FILE *fragGC[10],*fragAT[10],*fragCG[10],*fragTA[10];
/*******************************************************************/

/********************************stem**************************************/
char type_GC[10][100][5],type_AT[10][100][5],type_CG[10][100][5],type_TA[10][100][5];
char typen_GC[10][100][5],typen_AT[10][100][5],typen_CG[10][100][5],typen_TA[10][100][5];
float xx_GC[10][5][100],xx_AT[10][5][100],xx_CG[10][5][100],xx_TA[10][5][100];
int c[500],se[500][500],NGC[10],NAT[10],NCG[10],NTA[10],A_s;
...
void Input(void)
{
    int i,ii;
     ...
     {
                sprintf(filename,"fragment/A/A%d.pdb",ii); fragA[ii]=fopen(filename,"r+");
                sprintf(filename,"fragment/G/G%d.pdb",ii); fragG[ii]=fopen(filename,"r+");
                sprintf(filename,"fragment/C/C%d.pdb",ii); fragC[ii]=fopen(filename,"r+");
                sprintf(filename,"fragment/T/T%d.pdb",ii); fragT[ii]=fopen(filename,"r+");
    ...
    {
         i++;
         fscanf(fragT[ii],"%s %d %s %s %s %d %f %f %f %s %s %s\n",&atom[ii][i],&id[ii],type_T[ii][i],typen_T[ii][i],&chain[ii],&idnu[ii],&x_T[ii][1][i],&x_T[ii][2][i],&x_T[ii][3][i],&mark1,&mark2,&mark3);
    }
    NT[ii]=i;          
   fclose(fragA[ii]);
   fclose(fragG[ii]);
   ...
   for(ii=1;ii<=nm_frag;ii++)
     {
                ...
                sprintf(filename,"fragment/stem/AT/AT%d.pdb",ii); fragAT[ii]=fopen(filename,"r+");
                sprintf(filename,"fragment/stem/CG/CG%d.pdb",ii); fragCG[ii]=fopen(filename,"r+");
                sprintf(filename,"fragment/stem/TA/TA%d.pdb",ii); fragTA[ii]=fopen(filename,"r+");
   i=0;
   while(!feof(fragGC[ii]))
   {
    ...
    NGC[ii]=i;
  i=0;
   while(!feof(fragAT[ii]))
   {
        i++;
        fscanf(fragAT[ii],"%s %d %s %s %s %d %f %f %f %s %s %s\n",&atom[ii][i],&id[ii],type_AT[ii][i],typen_AT[ii][i],&chain[ii],&idnu[ii],&xx_AT[ii][1][i],&xx_AT[ii][2][i],&xx_AT[ii][3][i],&mark1,&mark2,&mark3);
    }
    NAT[ii]=i;
i=0;
   while(!feof(fragCG[ii]))
   {
    ...
    NCG[ii]=i;
   i=0;
   while(!feof(fragTA[ii]))
   {
        i++;
        fscanf(fragTA[ii],"%s %d %s %s %s %d %f %f %f %s %s %s\n",&atom[ii][i],&id[ii],type_TA[ii][i],typen_TA[ii][i],&chain[ii],&idnu[ii],&xx_TA[ii][1][i],&xx_TA[ii][2][i],&xx_TA[ii][3][i],&mark1,&mark2,&mark3);
    }
    NTA[ii]=i;
  } 
        ...
        for(ii=1;ii<=nm_frag;ii++)
        {
          if(strcmp(type[i],"G")==0) 
          {
                 ...
                 for(j=1; j<=3; j++)
                 {
                      ...
                      x_CG[ii][j][3]=x_G[ii][j][12];  
                      x_CG[ii][j][4]=x_G[ii][j][23];                 
...
//////////////A////////////
           ...
           { 
                  ...
                  { 
                             ...
                             x_CG[ii][j][3]=x_A[ii][j][12];  
                             x_CG[ii][j][4]=x_A[ii][j][22];                       
                  }
                  for(k=1;k<=N_AA[ii];k++)
                  {
...
//////////////C////////////
            else if(strcmp(type[i],"C")==0) 
            {
                     ...
                     for(j=1; j<=3; j++)
                     {
                           ...
                           x_CG[ii][j][3]=x_C[ii][j][12];  
                           x_CG[ii][j][4]=x_C[ii][j][20]; 
...
//////////////U////////////
             else if(strcmp(type[i],"T")==0) 
             {
                      N_AA[ii]=NT[ii];
                      for(j=1; j<=3; j++)
                      {
        
                              x_CG[ii][j][1]=x_T[ii][j][1]; 
                              x_CG[ii][j][2]=x_T[ii][j][6]; 
                              x_CG[ii][j][3]=x_T[ii][j][12]; 
                              x_CG[ii][j][4]=x_T[ii][j][21]; 
                            
                      }
                      for(k=1;k<=N_AA[ii];k++)
                      {
                             atom_AA[ii][k]=i/3+k-1; strcpy(type_AA[ii][k],type_T[ii][k]); strcpy(typen_AA[ii][k],typen_T[ii][k]); nucl_AA=i/3;
                             for(j=1;j<=3;j++)  
                             {
                                         x_AA[ii][j][k]=x_T[ii][j][k];
                             }
                       }
               }
               else 
              ...
              for(ii=1;ii<=nm_frag;ii++)
              {
                ...
                {                      
                  ...
                  {                     
                            ...
                            x_CG[ii][j][3]=xx_GC[ii][j][12]; 
                            x_CG[ii][j][4]=xx_GC[ii][j][23]; 
                            x_CG[ii][j][5]=xx_GC[ii][j][26];
                            x_CG[ii][j][6]=xx_GC[ii][j][31];
                            x_CG[ii][j][7]=xx_GC[ii][j][37];
                            x_CG[ii][j][8]=xx_GC[ii][j][45];                               
                   }
                   for(k=1;k<=N_AA[ii];k++)
                   {
                  ...
                  if(strcmp(type[i],"A")==0&&strcmp(type[sk],"T")==0) 
                  {
                     N_AA[ii]=NAT[ii];
                     for(j=1; j<=3; j++)
                     {                     
                           x_CG[ii][j][1]=xx_AT[ii][j][1]; 
                           x_CG[ii][j][2]=xx_AT[ii][j][6]; 
                           x_CG[ii][j][3]=xx_AT[ii][j][12];  
                           x_CG[ii][j][4]=xx_AT[ii][j][22];
                           x_CG[ii][j][5]=xx_AT[ii][j][25];
                           x_CG[ii][j][6]=xx_AT[ii][j][30]; 
                           x_CG[ii][j][7]=xx_AT[ii][j][36];
                           x_CG[ii][j][8]=xx_AT[ii][j][45];                        
                     }
                     for(k=1;k<=N_AA[ii];k++)
                     {
                        atom_AA[ii][k]=i/3+k-1; strcpy(type_AA[ii][k],type_AT[ii][k]); strcpy(typen_AA[ii][k],typen_AT[ii][k]); nucl_AA=i/3;
                        duange=22;nucl_AA1=sk/3;
                        for(j=1;j<=3;j++) { x_AA[ii][j][k]=xx_AT[ii][j][k]; }
                     }
                  }
                  if(strcmp(type[i],"C")==0&&strcmp(type[sk],"G")==0) 
                  {
                     ...
                     {                     
                           ...
                           x_CG[ii][j][3]=xx_CG[ii][j][12];  
                           x_CG[ii][j][4]=xx_CG[ii][j][20];
                           x_CG[ii][j][5]=xx_CG[ii][j][23];
                           x_CG[ii][j][6]=xx_CG[ii][j][28]; 
                           x_CG[ii][j][7]=xx_CG[ii][j][34];
                           x_CG[ii][j][8]=xx_CG[ii][j][45];                    
                     }
                     for(k=1;k<=N_AA[ii];k++)
                     {
                         atom_AA[ii][k]=i/3+k-1; strcpy(type_AA[ii][k],type_CG[ii][k]); strcpy(typen_AA[ii][k],typen_CG[ii][k]); nucl_AA=i/3;
                         duange=20;nucl_AA1=sk/3;
                         for(j=1;j<=3;j++) { x_AA[ii][j][k]=xx_CG[ii][j][k]; }
                     }
                  }
                  if(strcmp(type[i],"T")==0&&strcmp(type[sk],"A")==0) 
                  {
                     N_AA[ii]=NTA[ii];
                     for(j=1; j<=3; j++)
                     {                     
                           x_CG[ii][j][1]=xx_TA[ii][j][1]; 
                           x_CG[ii][j][2]=xx_TA[ii][j][6]; 
                           x_CG[ii][j][3]=xx_TA[ii][j][12];  
                           x_CG[ii][j][4]=xx_TA[ii][j][21];
                           x_CG[ii][j][5]=xx_TA[ii][j][24];
                           x_CG[ii][j][6]=xx_TA[ii][j][29]; 
                           x_CG[ii][j][7]=xx_TA[ii][j][35];
                           x_CG[ii][j][8]=xx_TA[ii][j][45];                
                     }
                     for(k=1;k<=N_AA[ii];k++)
                     {
                           atom_AA[ii][k]=i/3+k-1; strcpy(type_AA[ii][k],type_TA[ii][k]); strcpy(typen_AA[ii][k],typen_TA[ii][k]); nucl_AA=i/3;
                           duange=21;nucl_AA1=sk/3;
                           for(j=1;j<=3;j++) { x_AA[ii][j][k]=xx_TA[ii][j][k]; }
                     }
                  }
