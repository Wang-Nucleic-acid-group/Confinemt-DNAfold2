...
int stem_length[20];
float u1,u2;
FILE *fp,*fp1,*fp2,*fp3,*fp4,*fp5,*fp6,*fpsse;
int main()
{
    	int i;
   	...
   	fp2=fopen("stem_kissing.dat","w+");
    	...
    	while(!feof(fp)) 
    	{
     	...
     	secondary();  
     	return 0;    
     	fclose(fp);fclose(fp1);fclose(fp2);fclose(fp3);fclose(fp5),fclose(fpsse);
} 
float HB(int i1,int j1,float x1[1000],float y1[1000],float z1[1000])
{
 	...
 	if (d>=8.55&&d<=9.39)   //The pairing formation condition
 	{
		...
		if ((type[i1]=='A'&&type[j1]=='T')||(type[i1]=='T'&&type[j1]=='A'))  		{hb=BetaAT*A;} 
  		 UHB=hb/(1+2.66*(d-8.94)*(d-8.94)+1.37*((d0-12.15)*(d0-12.15)+(d01-12.15)*(d01-12.15))+0.464*((d1-13.8)*(d1-13.8)+(d11-13.8)*(d11-13.8))); 
  	}
 	else {UHB=0.0;}
 	return UHB;
}
