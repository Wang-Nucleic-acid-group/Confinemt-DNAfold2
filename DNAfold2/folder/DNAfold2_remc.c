...
#define bl 5.45
//#define RG 1.1           //TBI,the Rg of DNAs comparing with Rg of A-form helix;
...
float rt0[15]={25.0,31.0,37.0,45.0,54.0,64.0,74.0,86.0,98.0,110.0};
...
#pragma omp threadprivate(thread_num,a0,b0)
#pragma omp threadprivate(frac,fi,frac0,frac1,frac2,av,ed,Move,Bs,ucs,uucs)
//FILE *fp4;
int main()
{
	void  openfile(),closefile(),remc(),detailed();
    	...
    	for(rt=1;rt<=ex_times;rt++)
    	{
           	remc();
           	detailed();
           	printf("REMC || Folding DNA 3D structure from the sequence\n");
...
void replica()
{
 ...
 //printf("exchange time is %d, the thread is %d,  the DNA length =  %d, CG beads is %d \n",rrt[thread_num],thread_num,N,N0);
 	for(i=1;i<=N0;i++) 
  	{
...
void openfile()
{
    	...
    	void center();
     	...
     	for (i=0;i<N_thread;i++) 
     	{
      	...
      	while(!feof(fpcfig))
      	{
      		result=fscanf(fpcfig,"%d %d %d %d %d %f\n",&ct,&ca,&cb,&cc,&cd,&rrc);
      	}
	(void)result;
      	if(ca>=2000000)	
      	{
...
void Metropolis(void)
{
   	int i;
   	float p;
   	u=ulj+uN+us+uc+(ub+ue+ud)*0.5963+uco+ucs;
   	uu=uulj+uuN+uus+uuc+(uub+uue+uud)*0.5963+uuco+uucs;
...
float PCPC(int i1,float x1[1000],float y1[1000],float z1[1000])
{
	...
	c2=((z1[i1-2]-z1[i1-1])*(x1[i1-1]-x1[i1+1])-(x1[i1-2]-x1[i1-1])*(z1[i1-1]-z1[i1+1]));
 	...
 	ud0=1.0*((1-cos(di-2.5))+0.5*(1-cos(3.*(di-2.5)))); 
 	return ud0;
}
float CPCP(int i1,float x1[1000],float y1[1000],float z1[1000])
{
   	float c1,c2,c3,p1,p2,p3,e1,f1,pp1,g1,g2,g3,gg1,hh1,di,ud0=0.0;
     	...
     	ud0=0.5*((1-cos(di+2.95))+0.5*(1-cos(3.*(di+2.95))));
   	return ud0;
}
float CPCN(int i1,float x1[1000],float y1[1000],float z1[1000])
{
	float c1,c2,c3,p1,p2,p3,e1,f1,pp1,g1,g2,g3,gg1,hh1,di,ud0=0.0;
   	c1=((y1[i1-4]-y1[i1-2])*(z1[i1-2]-z1[i1-1])-(z1[i1-4]-z1[i1-2])*(y1[i1-2]-y1[i1-1]));
     	...
     	ud0=0.8*((1-cos(di+1.3))+0.5*(1-cos(3.*(di+1.3))));
   	return ud0;
}
float NCPC(int i1,float x1[1000],float y1[1000],float z1[1000])
{
 	float c1,c2,c3,p1,p2,p3,e1,f1,pp1,g1,g2,g3,gg1,hh1,di,ud0=0.0;
   	...
   	ud0=0.8*((1-cos(di-0.9))+0.5*(1-cos(3.*(di-0.9))));
   	return ud0;
}
/*  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  */
    	...
    	for (i=1;i<=N0;i++)  
    	{
     	...
     	{   	    
      		for(j=i+12;j<=N0;j=j+3)
             	{          		     
                    	if ((type[i]=='G'&&type[j]=='C')||(type[i]=='C'&&type[j]=='G')||(type[i]=='A'&&type[j]=='T')||(type[i]=='T'&&type[j]=='A'))
                    	{
